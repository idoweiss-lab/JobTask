...
def print_git_without_note(git_without_note):
    check_owner_no_comment = ["chdauto", "syspdbuild", "syspdbuild2"]#owners that dont need to print the missing commit
...
file_array = []#array that contains all the lines

file_array = load_file(FILE_PATH)

array_len = len(file_array)
